...
y_titles = np.array(['area_ratio', 'fine_detail_ratio', 'max_hole_diameter',
...
sorted_idx = [ 4,  3,  2,  7,  5,  9, 11,  8, 12, 10,  0,  6, 13,  1] # this matches the original sort -"14"
...
ax.plot(np.mean(oresult.importances[sorted_idx].T,axis=0),\
        np.arange(9),'k',marker='s',linestyle='')
ax.plot(np.mean(sresult.importances[sorted_idx].T,axis=0),\
        np.arange(9),'royalblue',marker='o',linestyle='')
ax.plot(np.mean(mresult.importances[sorted_idx].T,axis=0),\
        np.arange(9),'darkgoldenrod',marker='D',linestyle='')
ax.plot(np.mean(bresult.importances[sorted_idx].T,axis=0),\
        np.arange(9),'purple',marker='*',linestyle='',markersize=10)
...
plt.savefig("figure6_no_iat.png", dpi=600)
